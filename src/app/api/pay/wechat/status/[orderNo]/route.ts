...
import { queryPaymentStatus } from "@/lib/wechat";
...
/**
 * GET /api/pay/wechat/status/[orderNo]
 * 主动查询微信支付订单状态（异步通知丢失或状态未知时的兜底）。
...
export async function GET(
  ...
  try {
    ...
    // 5. 查询微信支付订单状态
    const queryResult = await queryPaymentStatus(
      orderNo,
      order.wechatTradeNo as string | undefined
    );
    ...
    if (queryResult.tradeStatus === "SUCCESS") {
      ...
      if (!queryResult.outTradeNo) {
        console.error(`[wechat] 查询成功但 outTradeNo 为空: orderNo=${orderNo}`);
        throw new ApiError(500, "微信支付查询结果缺少商户订单号");
      }
      if (queryResult.outTradeNo !== orderNo) {
        console.error(
          `[wechat] outTradeNo 不一致: query=${queryResult.outTradeNo}, order=${orderNo}`
        );
        throw new ApiError(400, "微信支付查询结果与订单号不一致");
      }
      if (!queryResult.tradeNo) {
        console.error(`[wechat] 查询成功但 tradeNo 为空: orderNo=${orderNo}`);
        throw new ApiError(500, "微信支付查询结果缺少交易号");
      }
      if (queryResult.totalAmount === undefined) {
        console.error(`[wechat] 查询成功但 totalAmount 缺失: orderNo=${orderNo}`);
        throw new ApiError(500, "微信支付查询结果缺少金额");
      }
      // 微信查询结果 totalAmount 是整数分的字符串表示（如 "1000"），直接解析
      const queryAmountFen = Number(queryResult.totalAmount);
      if (!Number.isSafeInteger(queryAmountFen) || queryAmountFen <= 0) {
        console.error(
          `[wechat] 查询金额格式非法: totalAmount=${queryResult.totalAmount}`
        );
        throw new ApiError(500, "微信支付查询金额格式非法");
      }
      const orderAmount = Number(order.amount);
      if (!Number.isSafeInteger(orderAmount) || orderAmount <= 0) {
        console.error(
          `[wechat] 订单金额非法: orderNo=${orderNo}, amount=${order.amount}`
        );
        throw new ApiError(500, "订单金额数据异常");
      }
      if (queryAmountFen !== orderAmount) {
        console.error(
          `[wechat] 查询金额与订单不一致: query=${queryAmountFen}分, order=${orderAmount}分, out_trade_no=${orderNo}`
        );
        throw new ApiError(400, "微信支付查询金额与订单不一致");
      }
      ...
      try {
        await creditRechargeOrder(
          orderNo,
          queryResult.tradeNo,
          `微信支付充值 ${order.packageId}（主动查询到账）`
        );
      } catch (e) {
        console.error(`[wechat] 主动查询到账失败: ${orderNo}`, e);
